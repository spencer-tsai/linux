...
#define REG_GPIO_CONFIG	0x80	/* ADT7490 GPIO config, value, direction, polarity */
...
/*
 * Sysfs support for ADT7490 GPIO1 and GPIO2
 *
 * gpio1_direction: 0=output, 1=input
 * gpio1_polarity: 0=active low, 1=active high
 * gpio1_value: read/write value (output or input state)
 * gpio2_direction: 0=output, 1=input
 * gpio2_polarity: 0=active low, 1=active high
 * gpio2_value: read/write value (output or input state)
 */

static ssize_t gpio1_direction_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	int reg = i2c_smbus_read_byte_data(client, REG_GPIO_CONFIG);
	if (reg < 0)
		return reg;
	return sprintf(buf, "%d\n", !!(reg & 0x80));
}

static ssize_t gpio1_direction_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	long val;
	...
	reg = i2c_smbus_read_byte_data(client, REG_GPIO_CONFIG);
	if (reg < 0) {
		mutex_unlock(&data->lock);
		return reg;
	}
	if (val)
		reg |= 0x80; // set bit 7 for input
	else
		reg &= ~0x80; // clear bit 7 for output
	i2c_smbus_write_byte_data(client, REG_GPIO_CONFIG, reg);
	mutex_unlock(&data->lock);
	return count;
}

static ssize_t gpio1_polarity_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	int reg = i2c_smbus_read_byte_data(client, REG_GPIO_CONFIG);
...
static ssize_t gpio1_polarity_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	long val;
	...
	if (val)
		reg |= 0x20; // set bit 5 for active high
	else
		reg &= ~0x20; // clear bit 5 for active low
	i2c_smbus_write_byte_data(client, REG_GPIO_CONFIG, reg);
	mutex_unlock(&data->lock);
	return count;
}

static ssize_t gpio1_value_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	int reg = i2c_smbus_read_byte_data(client, REG_GPIO_CONFIG);
...
static ssize_t gpio1_value_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	long val;
	...
	if (val)
		...
		reg &= ~0x08; // clear bit 3
	i2c_smbus_write_byte_data(client, REG_GPIO_CONFIG, reg);
	mutex_unlock(&data->lock);
	return count;
}

static ssize_t gpio2_direction_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	int reg = i2c_smbus_read_byte_data(client, REG_GPIO_CONFIG);
...
static ssize_t gpio2_direction_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	long val;
	...
	if (val)
		reg |= 0x40; // set bit 6 for input
	else
		reg &= ~0x40; // clear bit 6 for output
	i2c_smbus_write_byte_data(client, REG_GPIO_CONFIG, reg);
	mutex_unlock(&data->lock);
	return count;
}

static ssize_t gpio2_polarity_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	int reg = i2c_smbus_read_byte_data(client, REG_GPIO_CONFIG);
...
static ssize_t gpio2_polarity_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	long val;
	...
	if (val)
		reg |= 0x10; // set bit 4 for active high
	else
		reg &= ~0x10; // clear bit 4 for active low
	i2c_smbus_write_byte_data(client, REG_GPIO_CONFIG, reg);
	mutex_unlock(&data->lock);
	return count;
}

static ssize_t gpio2_value_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	int reg = i2c_smbus_read_byte_data(client, REG_GPIO_CONFIG);
...
static ssize_t gpio2_value_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct adt7475_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	long val;
	...
	if (val)
		...
		reg &= ~0x04; // clear bit 2
	i2c_smbus_write_byte_data(client, REG_GPIO_CONFIG, reg);
	mutex_unlock(&data->lock);
	return count;
}

static DEVICE_ATTR_RW(gpio1_direction);
static DEVICE_ATTR_RW(gpio1_polarity);
static DEVICE_ATTR_RW(gpio1_value);
static DEVICE_ATTR_RW(gpio2_direction);
static DEVICE_ATTR_RW(gpio2_polarity);
static DEVICE_ATTR_RW(gpio2_value);

static struct attribute *adt7490_gpio_attrs[] = {
	&dev_attr_gpio1_direction.attr,
	&dev_attr_gpio1_polarity.attr,
	&dev_attr_gpio1_value.attr,
	&dev_attr_gpio2_direction.attr,
	&dev_attr_gpio2_polarity.attr,
	&dev_attr_gpio2_value.attr,
	NULL,
};

static const struct attribute_group adt7490_gpio_attr_group = {
	.attrs = adt7490_gpio_attrs,
};
...
static int adt7475_probe(struct i2c_client *client)
{
	...
	/* Add GPIO sysfs group for ADT7490 */
	if (chip == adt7490) {
		data->groups[group_num++] = &adt7490_gpio_attr_group;
	}
